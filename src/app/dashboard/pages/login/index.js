...
import { AppContext, LOGIN_USER, LOGOUT_USER } from 'context/';
...
const Login = (props) => {
	...
	const onSubmitLogin = (values) => {
		setLoading(true);
		JwtService.loginAdmin(values.email, values.password)
			.then((tokenInfo) => {
				setLoading(false);
				setIsLogged(true);
		...
		<Content>
			<div className="login-main">
				<div className="login-header">
					<Row justify="center">
						<img alt="logo" width='100px' src={logoImg} style={{ marginBottom: "20px" }} />
					</Row>
						<Row justify="center">
							<span
								style={{
