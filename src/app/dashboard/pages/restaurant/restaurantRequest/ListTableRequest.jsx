...
export default function ListTableRequest({
  ...
  const [insufficientModel, setInsufficientModel] = useState(false);
  ...
  const onRestaurantRequestAccept = (index, row) => {
    if (row?._id) {
      setLoadingRows((prev) => [...prev, index]);
      api.restaurant
        .accept_restaurant_request(row._id)
        .then(onRefreshData)
        .catch(handleError)
        .finally(() => unsetLoading(index));
    } else notificationError("Invalid Restaurant ID");
  };

  const onRestaurantRequestReject = (index, row) => {
    if (row?._id) {
      setLoadingRows((prev) => [...prev, index]);
      api.restaurant
        .reject_restaurant_request(row._id)
        .then(onRefreshData)
        .catch(handleError)
        .finally(() => unsetLoading(index));
    } else notificationError("Invalid Restaurant ID");
  ...
  const getDocInsufficientModel = (index, row) => {
    const onDocumentIncomplete = () => {
      if (row?._id) {
        if (!rejectionMessage)
          return notificationError("Please set rejection Message");
        setLoadingRows((prev) => [...prev, index]);
        api.restaurant
          .document_incomplete_restaurant_request(row._id, {
            rejectionMessage,
          })
          .then(onRefreshData)
          .catch(handleError)
      ...
      >
        <Row justify="center" style={{width: '100%'}}>
          <Typography.Title level={5}>
            Please provide brief rejection message
          </Typography.Title>
          <Input.TextArea
      ...
      render: (rowId, row, idx) => {
        return (
          <Dropdown
            overlay={
              <Menu>
                {getDocInsufficientModel(idx, row)}
                <Menu.Item
                  onClick={() => {
                    Modal.confirm({
                      title: "Are you sure to accept the Restaurant request?",
                      icon: <ExclamationCircleOutlined />,
                      // content: 'Bla bla ...',
                      okText: "Accept",
                      onOk: () => onRestaurantRequestAccept(idx, row),
                      cancelText: "Discard",
                    });
                  }}
                  key="0"
                ...
                <Menu.Item
                  onClick={() => {
                    Modal.error({
                      title: "Are you sure to reject the Restaurant request?",
                      icon: <ExclamationCircleOutlined />,
                      okText: "Reject",
                      onOk: () => onRestaurantRequestReject(idx, row),
                      cancelText: "Discard",
                    });
                  }}
                  key="1"
                ...
                <Menu.Item
                  onClick={() => setInsufficientModel(true)}
                  key="2"
                  style={{
                    color: "#ff9800",
                  }}
                >
                  Insufficient Document (Reject)
                </Menu.Item>
                <Menu.Divider />

                <Menu.Item
                  onClick={() =>
                    history.push(routeURL.cms.restaurant_edit(row._id))
                  }
                  key="4"
                  style={{
                    color: "#2196f3",
