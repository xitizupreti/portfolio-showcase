...
import ValidateRider from "./ValidateRider";
...
export default function ListTableRequest({
  ...
  const onRiderRequestReject = (index, row) => {
    if (row?.rider?._id) {
      setLoadingRows((prev) => [...prev, index]);
      api.rider
        .reject_rider_request(row.rider._id)
        .then(onRefreshData)
        .catch(handleError)
        .finally(() => unsetLoading(index));
    } else notificationError("Invalid Rider ID");
  ...
  const getDocInsufficientModel = (index, row) => {
    const onDocumentIncomplete = () => {
      if (row?.rider?._id) {
        ...
        setLoadingRows((prev) => [...prev, index]);
        api.rider
          .document_incomplete_rider_request(row.rider._id, {
            rejectionMessage,
          })
          .then(onRefreshData)
          .catch(handleError)
            ...
            overlay={
              <Menu>
                ...
                <Menu.Item
                  onClick={() => {
                    Modal.confirm({
                      title: "Are you sure to accept the Rider request?",
                      icon: <ExclamationCircleOutlined />,
                      // content: 'Bla bla ...',
                      okText: "Accept",
                      onOk: () => onRiderRequestAccept(idx, row),
                      cancelText: "Discard",
                    });
                  }}
                  key="0"
                ...
                <Menu.Item
                  onClick={() => {
                    Modal.error({
                      title: "Are you sure to reject the Rider request?",
                      icon: <ExclamationCircleOutlined />,
                      okText: "Reject",
                      onOk: () => onRiderRequestReject(idx, row),
                      cancelText: "Discard",
                    });
                  }}
                  key="1"
                ...
                <Menu.Item
                  onClick={() =>
                    history.push(routeURL.cms.rider_request_edit(row.rider._id))
                  }
                  key="4"
                  style={{
                    color: "#2196f3",
